...
SCORES = ['mean_F1', 'IC', 'IB', 'VC', 'VB']
BASELINES = [0.47369345142021646, 0.4257722385427191, 116, 0.5372817904136711, 24]
FILTERS = ["log_prob_ratio", "log_prob_sum", "log_prob"]


def compare(results_path, percentiles, filters):
    for i, score in enumerate(SCORES):
        baseline = BASELINES[i]
        compare_score(results_path, percentiles, filters, score, baseline)


def compare_score(results_path, percentiles, filters, score_name='mean_F1',
                  baseline=0.47369345142021646):

    assert all(percentiles[i] <= percentiles[i+1]
               for i in range(len(percentiles)-1)), 'percentiles must be sorted'
    ...
    for percentile in percentiles:
        for criteria in filters:
            ...
            # json_path = join(results_path, f'{criteria}_{percentile}_k=f.json')
    ...
    for key, values in criteria_scores.items():
        if len(values) > 0:
            if key == 'baseline':
                ax.plot(percentiles, values, label=key, linestyle='dashed')
            else:
                ax.plot(percentiles, values, label=key)
    legend = ax.legend()
    plt.title(f'{score_name}')
    fig_path = join(results_path, f'compare_filter_{score_name}.png')
...
if __name__ == '__main__':
    ...
    compare(args.results_path, args.percentiles, args.criteria)
